...
def loadMovieLens(path=''):
	data = {}
	i = 0
	f = codecs.open(path + sys.argv[1], 'r', 'ascii')
	for line in f:
		fields = line.split('\t')
		user = fields[0]
		movie = fields[1]
...
def normalizeData( data ):
	...
	for user in data:
		...
		for (movie,rating) in data[user].iteritems():
			userNormalized[movie] = (rating - userAvg)
		normalized[user] = userNormalized
...
def calcSim (user, userOther, data):
	...
	for key in rating1:
		if key in rating2:
			...
			both_rating1.append(rating1[key])
			both_rating2.append(rating2[key])
	if n == 0:
		return 0
	else:

		for value in both_rating1.itervalues():
			sum_x2 += value**2
		for value in both_rating2.itervalues():
			sum_y2 += value**2
...
#		print "total ?"
#		print str(el[1]) + " " + str(data[el[0]][movie]) + " " + str(total)
#		print "denominator" + str(denominator_sum) 
	div = 0
...
def mainFunction(path=''):
	...
	simMatrix = createSimMatrix(data)
 	output_fileName = sys.argv[1]+"_prediction"+".txt"
        fw = open(output_fileName,'w')
	for i in range (1,51):
		f = codecs.open(path + sys.argv[2], 'r', 'ascii')
		cnt = 0
		dif = 0
		sum = 0
		for line in f:
			...
			movie = fields[1]
			rating = int(fields[2].strip().strip('"'))
			calculated = int((calculateRating(user, movie, data, simMatrix, i)))			
			result = str(user)+ "\t"+ str(movie) + "\t" + str(calculated) + "\n"
			fw.write(result)		
